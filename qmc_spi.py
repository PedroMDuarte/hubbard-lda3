...
def spi_bulk( r111, n111, mu111, T, t111, U111, **kwargs ): 
    ...
    sthextents = kwargs.get('sthextents', 100.) 
    ...
    for i in range(len(r111)):
        ...
        title_text = r'$(U/t)_{{0}}={:0.2f}$,\ '.format(U/t) \
            + '$(T/t)_{{0}}={:0.2f}$,\ $\mu={:.2f}$,\ $r={:.2f}$'.\
                       format(T/t,mu,r111[i]) + '\n'
        ...
        if posr111[i] <= sthextents:
            # Find the Sth
            result = qmc.find_closest_qmc( U=Uval, T=Tval, mu=mu, \
                         title_text = title_text, radius=r111[i], QTY='sth', 
                         error_nan=True )
            if result is None:
                print "Had problems finding Spi for " +  \
                  " U={:02d}, T={:0.2f}".format(int(Uval), Tval)
    ...
    sthbulk =  integrate_sphere( r111, sth ) / number 
    ...
    if do_kappa:
        ...
        return spibulk, spi, sthbulk, sth, overall_entropy, entropy, lda_number, density
