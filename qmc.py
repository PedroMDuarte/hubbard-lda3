...
def find_closest_qmc( U=8, T=0.67, mu=4.0, **kwargs):
    ...
    msg0 = 'U={:0.2f}, T={:0.2f}, mu={:0.2f}'.format(U,T,mu)
    ...
    if QTY == 'spi':
        ...
        datadir = basedir + 'COMB_Final_Sth/'
    elif QTY == 'entropy':
        datadir = basedir + 'COMB_Final_Entr/'
    elif QTY == 'density':
        datadir = basedir + 'COMB_Final_Dens/'
    elif QTY == 'kappa':
        datadir = basedir + 'COMB_Final_Dens/'
    else:
    ...
    # Truncate temperature for density and Sth calculation
    if QTY == 'density' or QTY =='sth' :
        if U < 1.:
            Tlim = 0.50 - U*0.13
            if mu < U/2. and T < Tlim: T=Tlim
        elif U < 2.:
            Tlim = 0.45 - U*0.15/2.
            if mu < U/2. and T < Tlim: T=Tlim
        elif U < 4.:
            Tlim = 0.3  + (U-2.) * (0.38-0.3) / (4.-2.) 
            if mu < U/2. and T < Tlim: T=Tlim
        elif U < 8.:
            Tlim = 0.37  + (U-4.) * (0.38-0.27) / (8.-4.) 
            if mu < U/2. and T < Tlim: T=Tlim
        else:
            if mu < U/2. and T < 0.4: T=0.4
    # Truncate temperature for entropy calculation
    if QTY == 'entropy':
        if U < 1.:
            Tlim = 0.50 - U*0.13
            if mu < U/2. and T < Tlim: T=Tlim
    ...
    for u in us:    
        ...
        if QTY == 'spi' or QTY == 'kappa':
            fname = datadir + 'U{U:02d}/T*dat'.format(U=int(u))
            fs = sorted(glob.glob(fname))
            Ts = [ float(f.split('T')[1].split('.dat')[0]) for f in fs ]
        elif QTY=='density' or QTY == 'kappa':
            fname = datadir + 'U{U:02d}/n*dat'.format(U=int(u))
            fs = sorted(glob.glob(fname))
            
            Ts = []
            for f in fs:
                fname = os.path.basename(f) 
                Ts.append( float(fname.split('n')[1].split('.dat')[0] ))
        elif QTY=='entropy':
            ...
            fname = datadir + 'U{U:02d}/H*dat'.format(U=int(u))
            fs = sorted(glob.glob(fname))
            Ts = [ float(f.split('H')[1].split('.dat')[0]) for f in fs ]
        ...
        # debug:
        #if np.abs(U-4.20)<0.02 and np.abs(T-0.36)<0.02 and \
        #   np.abs(mu+2.11)<0.02:
        #    print
        #    print msg0 
        #    print "Looking at QTY=",QTY, " for U=",u
        #    print fs
        #    print Ts_g
        #    print Ts_l
        #    print Ts
...
#        
    ...
    try :
        ...
        print "==============================="
        print " error summary " 
        print kwargs.get('title_text', 'no-title')
        print msg0 
        print "basedat  = ", basedat 
...
def qmc_dens( T, t, mu, U, ignoreLowT=False, verbose=True):
    U_ = U/t 
    T_ = T/t 
    mu_ = mu/t   
    ...
    for i in range( len(mu_)):
        result[i] = find_closest_qmc( U=U_[i], T=T_[i], mu=mu_[i], \
                     QTY='density' ) 
    return result    
 
def qmc_entr( T, t, mu, U, ignoreLowT=False, verbose=True):
    U_ = U/t 
    T_ = T/t 
    mu_ = mu/t   
...
def qmc_spi( T, t, mu, U, ignoreLowT=False, verbose=True):
    U_ = U/t 
    T_ = T/t 
    mu_ = mu/t   
...
def qmc_cmpr( T, t, mu, U, ignoreLowT=False, verbose=True):
    U_ = U/t 
    T_ = T/t 
    mu_ = mu/t   
